...
namespace a2
{
    TrackBall::TrackBall(glm::vec3 position, float r) :
        mPosition(position),
        radius(r),
        ...
        mIndexBuffer(GL_ELEMENT_ARRAY_BUFFER)
    {
        ...
        {
            {std::string(lab2::ShaderDirectory) + "Ball.vs.glsl", GL_VERTEX_SHADER},
            {std::string(lab2::ShaderDirectory) + "Ball.fs.glsl", GL_FRAGMENT_SHADER}
        };
        ...
        atlas::math::Matrix4 const& view)
    {
        ...
        const math::Vector metalGray{ 0.5f, 0.46f, 0.69f };
        
//        set position
        mModel = glm::translate(math::Matrix4(1.0f), mPosition);
        mModel = glm::scale(mModel, glm::vec3(0.5*radius,0.5*radius,0.5*radius));
